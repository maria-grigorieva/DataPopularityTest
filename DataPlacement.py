...
#
# # Get current number of dataset replicas at clouds
# curr_replicas_info = []
# for k,v in df.groupby('datasetname')['attempt_start','ds_replicas_number','ds_replicas_clouds']:
...
for i,int_df in enumerate(intervals_df):
    ...
    for d in datasetnames:
        ...
        if curr_dataset_df.empty != True:
            ...
            for c in clouds:
                curr_cloud_df = curr_dataset_df[curr_dataset_df['site_cloud']==c]
...
datasets_cloudsAF = agg_intervals_df.groupby(['datasetname','cloud'])['weighted_accesses'].sum()/n_intervals
datasets_cloudsAF = datasets_cloudsAF.reset_index()
datasets_cloudsAF.rename(columns={'weighted_accesses':'datasetAF_cloud'}, inplace=True)

datasets_n_replicas_df = pd.read_csv('datasetsAF.csv')

# Calculate Avg Access Frequency of Each Dataset in All Clouds
datasetsAF = datasets_cloudsAF.groupby('datasetname')['datasetAF_cloud'].sum()
datasetsAF = datasetsAF.reset_index()
datasetsAF.rename(columns={'datasetAF_cloud':'datasetAF_all_clouds'}, inplace=True)
datasetsAF = pd.merge(datasetsAF, datasets_cloudsAF, how='left', left_on=['datasetname'], right_on=['datasetname'])
datasetsAF = pd.merge(datasetsAF, datasets_n_replicas_df, how='left', left_on=['datasetname'], right_on=['datasetname'])
datasetsAF['replicas_at_cloud'] = round(datasetsAF['optimal_n_replicas']*(datasetsAF['datasetAF_cloud']/datasetsAF['datasetAF_all_clouds']))
#datasetsAF['decision'] = datasetsAF['optimal_n_replicas']-datasetsAF['curr_n_replicas']

print(datasetsAF)
datasetsAF.to_csv('datasetsAF_clouds.csv')
print('saved!')